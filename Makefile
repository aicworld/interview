# Copyright 2023 aicworld. All rights reserved.
...
ROOT_PACKAGE=github.com/aicworld/interview
...
EXCLUDE_TESTS=github.com/aicworld/interview/test
...
build: # go.build.verify $(addprefix go.build., $(addprefix $(PLATFORM)., $(BINS)))
	...
	@cd $(SERVER_DIR) && CGO_ENABLED=0 GOOS=$(OS) GOARCH=$(ARCH) $(GO) build $(GO_BUILD_FLAGS) -o $(BIN_DIR)/platforms/interview $(ROOT_DIR)/server/main.go
...
copyright-add: tools.verify.addlicense
	...
	@$(TOOLS_DIR)/addlicense -y $(shell date +"%Y") -v -c "interview & OpenIM open source community." -f $(LICENSE_TEMPLATE) $(CODE_DIRS)
...
# example: ./coscli  cp/sync -r /root/workspaces/aicworld/interview/ cos://aicworld-1306374445/code/ -e cos.ap-hongkong.myqcloud.com
